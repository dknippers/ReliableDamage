...
delegate WeaponDamageValue GetBonusEffectDamageValueDelegate(XComGameState_Ability AbilityState, XComGameState_Item SourceWeapon, StateObjectReference TargetRef);
delegate ModifyDamageValueDelegate(out WeaponDamageValue DamageValue, Damageable Target, out array<Name> AppliedDamageTypes);
...
function WeaponDamageValue GetBonusEffectDamageValue(XComGameState_Ability AbilityState, XComGameState_Item SourceWeapon, StateObjectReference TargetRef)
{
	...
	return GetBonusEffectDamageValueFn(AbilityState, SourceWeapon, TargetRef);
}
...
{
    // We call the stored delegate function of the Source (see Clone function)
	ModifyDamageValueFn(DamageValue, Target, AppliedDamageTypes);
}
...
simulated protected function OnEffectAdded(const out EffectAppliedData ApplyEffectParameters, XComGameState_BaseObject kNewTargetState, XComGameState NewGameState, XComGameState_Effect NewEffectState)
{
	...
	if( kNewTargetDamageableState != none )
	{
		...
		{
			iModifiedHitChance = StandardAim_RD.GetModifiedHitChanceForCurrentDifficulty(Player, iBaseHitChance);
...
simulated function GetDamagePreview(StateObjectReference TargetRef, XComGameState_Ability AbilityState, out WeaponDamageValue MinDamagePreview, out WeaponDamageValue MaxDamagePreview, out int AllowsShield)
{
	...
	// Default behavior
	super.GetDamagePreview(TargetRef, AbilityState, MinDamagePreview, MaxDamagePreview, AllowsShield);
	...
	{
		iModifiedHitChance = StandardAim_RD.GetModifiedHitChanceForCurrentDifficulty(Player, iHitChance);
	...
	// We need the armor value and rupture value of the target separately for proper calculations
	if(History != None)
	{
...
simulated function AddX2ActionsForVisualization(XComGameState VisualizeGameState, out VisualizationTrack BuildTrack, name EffectApplyResult)
{
	...
	Destructible = XComGameState_Destructible(BuildTrack.StateObject_OldState);
